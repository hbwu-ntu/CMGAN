...
parser.add_argument("--cut_len", type=int, default=16000*2, help="cut length, default is 2 seconds in denoise "
                                                                 "and dereverberation")
parser.add_argument("--data_dir", type=str, default='/Users/hbwu/Downloads/PublicData/noisy-vctk-16k',
...
class Trainer:
    def __init__(self, train_ds, test_ds, ):
        ...
        self.model = TSCNet(num_channel=64, num_features=self.n_fft // 2 + 1)
        summary(
        ...
        for epoch in range(args.epochs):
            ...
            for idx, batch in enumerate(self.train_ds):
                ...
                if (step % args.log_interval) == 0:
                    logging.info(
                        template.format(epoch, step, loss, disc_loss)
                    )
            gen_loss = self.test()
            path = os.path.join(
                args.save_model_dir,
